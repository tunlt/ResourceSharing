 ...
 * @author Tu
 */
@WebServlet(name = "RejectRequestServlet", urlPatterns = {"/RejectRequestServlet"})
...
private final String MANAGEREQUEST = "managerequest.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = "";
        try{
        if(responeMessage.isEmpty()){
            request.setAttribute("MESSAGE_MANAGE", "responeMessage not empty!");
            url = MANAGEREQUEST;
        }else{
            int quantitystock = itemdao.getQuantityByItemID(itemid);
            insert = bookingdetaildao.insertBookingDetail(itemid, censorname.getUsername(),responeMessage,booking.getBookingid());
            updatestatus = bookingdao.UpdateStatusbybookingID(booking.getBookingid(), "Deleted");
            updatequantity = itemdao.UpdateQuantitybyItemID(itemid, quantity+quantitystock);
            if(insert && updatequantity && updatestatus){                              
                request.setAttribute("MESSAGE_MANAGE", "Reject successfully!");
                url = SEARCHREQUESTADMIN;
            }else{
                request.setAttribute("MESSAGE_MANAGE", "Reject Fail!");
                url = MANAGEREQUEST;
            }
        }
